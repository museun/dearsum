...
pub fn repeat(target: Duration, relative_time: Duration, apply: Easing, offset: f32) -> f32 {
    let t = relative_time.as_secs_f32() * 1e3 + offset;
    let s = target.as_secs_f32() * 1e3;
    let x = t % s;
    apply(x / s)
}
...
pub fn elastic_in(t: f32) -> f32 {
    match t {
        ...
        t if 1.0 <= t => 1.0,
        _ => {
            let f = 10.0f32.mul_add(t, -10.0).exp2();
...
pub fn bounce_out(t: f32) -> f32 {
    ...
    match t {
        ...
        t if t < 2.0 / T => S.mul_add((t - 1.5 / T).powi(2), 3.0 / 4.0),
        t if t < 2.5 / T => S.mul_add((t - 2.25 / T).powi(2), 15.0 / 16.0),
        _ => S.mul_add((t - 2.625 / T).powi(2), 63.0 / 64.0),
    }
}
