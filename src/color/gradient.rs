...
pub fn gradient(t: f32, offset: Vec3, amp: Vec3, freq: Vec3, phase: Vec3) -> Rgba {
    let v = offset + amp * ((freq * t + phase) * TAU).cos();
    Rgba::from_float([v.x, v.y, v.z, 1.0])
}
...
impl Gradient {
    ...
    pub fn to_color(&self, t: f32) -> Rgba {
        gradient(t, self.offset, self.amp, self.freq, self.phase)
    }
}
